...
// Each tile renders a live first-page preview; clicking opens a viewer that can
// rename or delete. Uploads are auto-titled server-side, so the "Add" tile just
// fires the file straight at /api/upload-job-description with no title field.
import { useCallback, useEffect, useRef, useState } from 'react';
import { C, FONT, RADIUS, BORDER } from '../funnel/tokens';

// A JD row plus the server-computed public URL (the client can't build storage
// URLs itself — no NEXT_PUBLIC_SUPABASE_URL).
type JobItem = {
  id: string;
  title: string;
  file_path: string;
...
export default function JobLibrary({
  ...
  // The JD currently open in the viewer sub-modal (null = closed).
  const [active, setActive] = useState<JobItem | null>(null);
  const fileRef = useRef<HTMLInputElement>(null);

  // (Re)load the library. Called on mount and after every upload/rename/delete.
  ...
  // Auto-upload on file select — the server parses the title from the PDF.
    ...
    <div style={styles.overlay} onClick={onClose}>
      ...
      >
        <div style={styles.header}>
          <div>
            <h2 style={styles.title}>Job Descriptions</h2>
            <p style={styles.sub}>
              Your uploaded JD library — click any to view or edit
            </p>
          </div>
        ...
        <div style={styles.scroll}>
          {loading ? (
            ...
            <div style={styles.grid}>
              {/* Add tile — hidden file input, auto-uploads on select. */}
              <button
                type="button"
              ...
              {jobs.length === 0 && !uploading && (
                <div style={styles.empty}>No job descriptions yet.</div>
              )}
              ...
              {jobs.map((job) => (
                ...
                >
                  <div style={styles.previewBox}>
                    <object
                      data={`${job.public_url}#toolbar=0&navpanes=0&view=FitH`}
                      type="application/pdf"
                      style={styles.previewObject}
                      aria-label={job.title}
                    />
                    {/* Transparent overlay so the whole tile is clickable and the
                        embedded PDF doesn't swallow the click. */}
                    <div style={styles.previewOverlay} />
                  </div>
                  <span style={styles.tileTitle}>{job.title}</span>
                </button>
              ))}
            </div>
          )}
  ...
  const save = async () => {
    ...
    try {
      const res = await fetch('/api/job-descriptions', {
        ...
        body: JSON.stringify({ id: job.id, title: title.trim() }),
      });
      const data = await res.json();
      ...
      >
        ...
        {msg && (
          <div
            style={{
              ...styles.viewerMsg,
              color: msg.startsWith('Saved') ? C.green : C.red,
            }}
          >
            {msg}
          </div>
        ...
        <iframe src={job.public_url} style={styles.viewerFrame} title={job.title} />
      </div>
    </div>
  );
}
